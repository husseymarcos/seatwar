...
export function comparePoints(
  ...
  driverA: number,
  driverB: number,
): { side: RivalrySide; driverAPoints: number | null; driverBPoints: number | null } {
  const { a, b } = pickPointsByDriver(standings, driverA, driverB);
  if (a == null && b == null) return { side: "tie", driverAPoints: null, driverBPoints: null };
...
export function buildWhoIsWinningResult(input: {
  ...
  const { standings, raceSessionsResults, qualifyingSessionsResults, driverA, driverB } =
    input;
  ...
  const qualAgg = aggregateHeadToHead(qualifyingSessionsResults, driverA, driverB);
...
export function winSharesFromWhoIsWinning(
  result: WhoIsWinningResult,
): { driverA: number; driverB: number } {
...
export async function getWhoIsWinning(
  ...
  const year =
    scope.mode === "season" ? scope.year : currentYear;

  let allSessions: SessionSummary[];
  if (scope.mode === "season") {
    allSessions = await getSessions({ year: scope.year }, behavior);
  } else {
    const [thisYear, prevYear] = await Promise.all([
      getSessions({ year }, behavior),
      getSessions({ year: year - 1 }, behavior),
    ]);
    const byKey = new Map<number, SessionSummary>();
    for (const s of prevYear) byKey.set(s.session_key, s);
    for (const s of thisYear) byKey.set(s.session_key, s);
  ...
  const [standings, ...raceResults] = await Promise.all([
    ...
    ...raceKeys.map((sessionKey) =>
      getSessionResults({ sessionKey }, behavior),
    ),
  ]);

  const qualifyingResults = await Promise.all(
    qualKeys.map((sessionKey) => getSessionResults({ sessionKey }, behavior)),
  );
